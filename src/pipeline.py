...
from diffusers.models.transformers.lumina_nextdit2d import LuminaNextDiT2DModel
...
from .model import LuminaT2IAdapter
from ..utils import pil_to_pt
...
class LuminaAdapterPipeline(DiffusionPipeline):
    r"""
    ...
    def encode_prompt(
        ...
        **kwargs,
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_attention_mask = negative_text_inputs.attention_mask.to(
                device
            )
            ...
            negative_prompt_embeds = negative_prompt_embeds.to(
                dtype=negative_dtype, device=device
            )
            # duplicate text embeddings and attention mask for each generation per prompt, using mps friendly method
    ...
    def __call__(
        ...
        adapter_conditioning_scale: int = 0.8,
        ...
        if proportional_attn:
            cross_attention_kwargs["base_sequence_length"] = (
                self.default_image_size // 16
            ) ** 2
        ...
        block_hidden_states = self.adapter(image, image_rotary_emb)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                current_timestep = (
                    1 - current_timestep / self.scheduler.config.num_train_timesteps
                )
                ...
                if do_classifier_free_guidance:
                    noise_pred_eps, noise_pred_rest = (
                        noise_pred[:, :3],
                        noise_pred[:, 3:],
                    )
                    ...
                    noise_pred_eps = torch.cat(
                        [noise_pred_half, noise_pred_half], dim=0
                    )
